...
# ---------------------------------------------------------------------
# Geometrický witness: Z2 branch + θ-scan (WRATH/BOOST kompatibilní)
# ---------------------------------------------------------------------

def extract_assignment_from_ipc_mode(
    clauses: List[List[int]],
    nvars: int,
    clause_phasors: np.ndarray,
    theta: float,
    ...
    """Stejné jako extract_assignment_from_ipc, ale s volitelnou Z2 metrikou.

    mode='lin':  cos(φ-θ)
    mode='z2' :  cos(2(φ-θ))  (U(1) slepota → čistá Z2 geometrie)
    """
    if nvars <= 0:
        return np.zeros(0, dtype=bool), np.zeros(0, dtype=np.float64)

    inc = [[] for _ in range(nvars)]
    for j, cl in enumerate(clauses):
        for lit in cl:
            v = abs(lit) - 1
            if 0 <= v < nvars:
                inc[v].append((j, +1 if lit > 0 else -1))

    phi = np.angle(clause_phasors)
    amp = np.abs(clause_phasors)
    w = clause_weights.astype(np.float64, copy=False)
    use_z2 = (str(mode).lower() == 'z2')

    score = np.zeros(nvars, dtype=np.float64)
    for v in range(nvars):
        s = 0.0
        for j, lit_sign in inc[v]:
            offset = (1.0 - w[j]) * 0.1
            th = float(theta) + offset if lit_sign > 0 else float(theta) + math.pi - offset
            ang = float(wrap_pi(phi[j] - th))
            contrib = math.cos(2.0 * ang) if use_z2 else math.cos(ang)
            s += w[j] * amp[j] * contrib
        score[v] = s
...
def theta_scan_grid(theta0: float, span: float, n: int) -> np.ndarray:
    n = int(max(3, n))
    span = float(abs(span))
    if span <= 0:
        return np.array([float(theta0)], dtype=np.float64)
    return np.linspace(float(theta0) - span, float(theta0) + span, n, dtype=np.float64)


def select_best_witness_geometric(
    clauses: List[List[int]],
    nvars: int,
    a: np.ndarray,
    w: np.ndarray,
    ...
    scan_n: int = 73,
    refine_n: int = 81,
    refine_span: float = 0.35,
) -> Tuple[int, float, str, bool, np.ndarray, np.ndarray, Tuple[float,float,float]]:
    """Vrátí nejlepší svědek přes: (lin/z2) × (θ-scan) × (globální flip).

    Klíč je lexikograficky: (UNSAT, -mean(score), -min(score)).
    """
    # Z2 branch: θ a θ+π jsou ekvivalentní v kvadrátu → skenujeme kolem obou.
    base_thetas = [float(theta_hint_z2), float(wrap_pi(theta_hint_z2 + math.pi)), float(theta_hint_u1)]
    # odstranění duplicit
    uniq=[]
    for t in base_thetas:
        if not any(abs(float(wrap_pi(t-u)))<1e-9 for u in uniq):
            uniq.append(float(t))

    best = None

    def consider(th: float, mode: str, do_flip: bool):
        nonlocal best
        assign, score = extract_assignment_from_ipc_mode(clauses, nvars, clause_phasors=a, theta=th, clause_weights=w, mode=mode)
        if do_flip:
            assign = np.logical_not(assign)
            score = -np.asarray(score)
        uns = count_unsat(clauses, assign)
        arr = np.ravel(np.asarray(score, dtype=np.float64))
        mean = float(arr.mean()) if arr.size else 0.0
        mn = float(arr.min()) if arr.size else 0.0
        key = (int(uns), -mean, -mn)
        if (best is None) or (key < best[0]):
            best = (key, float(th), str(mode), bool(do_flip), assign, score, (mean, mn, float(arr.std()) if arr.size else 0.0))

    # hrubý scan: kolem každého hintu; pro z2 i lin
    for t0 in uniq:
        for th in theta_scan_grid(t0, math.pi, int(scan_n)):
            for mode in ('lin','z2'):
                consider(float(th), mode, False)
                consider(float(th), mode, True)

    # jemný scan: kolem vítězného theta
    if best is not None and refine_n and refine_span:
        _key, th_star, _mode, _flip, *_ = best
        for th in theta_scan_grid(th_star, float(refine_span), int(refine_n)):
            for mode in ('lin','z2'):
                consider(float(th), mode, False)
                consider(float(th), mode, True)

    key, th, mode, flip, assign, score, stats = best
    return key[0], th, mode, flip, assign, score, stats
...
def run(
    ...
    theta, beta, delta, a = ipc_metrics(Z_lock, u, m=m)    # ------------------ CNF witness extraction (geometrie: Z2 branch + θ-scan) ------------------
    witness = {}
    if cnf_path:
        # Koherence diagnostika (U1 vs Z2)
        S1_w = np.sum(a)
        sum_abs_w = float(np.sum(np.abs(a)) + 1e-12)
        coh_u1_w = float(np.abs(S1_w) / sum_abs_w)

        S2_w = np.sum(a * a)
        sum_abs2_w = float(np.sum(np.abs(a) ** 2) + 1e-12)
        coh_z2_w = float(np.abs(S2_w) / sum_abs2_w)

        theta_u1 = float(np.angle(S1_w)) if np.abs(S1_w) > 1e-12 else 0.0
        theta_z2 = float(0.5 * np.angle(S2_w)) if np.abs(S2_w) > 1e-12 else theta_u1

        # Bimodal index (pokud je Z2 tvrdé a U1 mizí, lineární čtení je slepé)
        bimodal_index = float((coh_z2_w + 1e-12) / (coh_u1_w + 1e-12))

        # Geometrický výběr svědka: (lin/z2) × θ-scan × globální flip
        unsat_wit, theta_used, readout_mode, flip_used, assign_wit, score_wit, (score_mean, score_min, score_std) =             select_best_witness_geometric(
                clauses=clauses,
                nvars=nvars,
                a=a,
                w=w,
                theta_hint_u1=theta_u1,
                theta_hint_z2=theta_z2,
                scan_n=73,
                refine_n=81,
                refine_span=0.35,
            )

        # Finální sanity check (sjednocený, odpálí index chyby okamžitě)
        unsat_check = check_cnf(clauses, assign_wit)
        print("CNF check unsat:", unsat_check)

        # Drive statistiky (na zvoleném θ)
        phi_w = np.angle(a)
        amp_w = np.abs(a).astype(np.float64, copy=False)
        align_w = np.cos(wrap_pi(phi_w - float(theta_used))).astype(np.float64, copy=False)
        phi_w_use = phi_w.copy()
        flip_w = (align_w < 0.0)
        if np.any(flip_w):
            phi_w_use[flip_w] = wrap_pi(phi_w_use[flip_w] + math.pi)
            align_w = np.cos(wrap_pi(phi_w_use - float(theta_used))).astype(np.float64, copy=False)
        ...
        witness = {
            ...
            "score_stats": {
                "min": float(np.min(np.ravel(np.asarray(score_wit)))) if np.size(score_wit) else 0.0,
                "max": float(np.max(np.ravel(np.asarray(score_wit)))) if np.size(score_wit) else 0.0,
                "mean": float(score_mean),
                "std": float(score_std),
            },
            "geom_diag": {
                "coherence_u1": float(coh_u1_w),
                "coherence_z2": float(coh_z2_w),
                "bimodal_index": float(bimodal_index),
            },
        }
