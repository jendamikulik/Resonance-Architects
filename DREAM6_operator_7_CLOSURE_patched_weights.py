...
def get_optimal_weights_cvxopt(J: int, delta_min: float = 12.0, delta_max: float = 1000.0) -> np.ndarray:
    """
    CVXOPT dyadic tail-weights — imported from RH_MADNESS_5_ROBUSTSCORE logic.

    Solve QP to suppress tail energy:
      minimize   w_sq^T K_tail w_sq
      subject to w_sq >= 0, 1^T w_sq = 1
    then set w = sqrt(w_sq) and normalize so max(w)=1.

    If cvxopt (or scipy quad) is unavailable, falls back to uniform weights.
    """
    J = int(J)
    if J <= 0:
    ...
    try:
        ...
        m = float(np.max(w)) if float(np.max(w)) > 0 else 1.0
        return w / m
    except Exception:
        return np.ones(J, dtype=float)
...
def closure_fuse_cutoff(values: np.ndarray, threshold_percentile: float = 95.0,
    ...
    """
    CLOSURE-FUSE protokol (singulární řez).

    Z CLOSURE.pdf (Section 6):
    1. Trigger: detekce kdy |gradient| → ∞
    2. Cut: aplikuj Soft-ABS regularizaci
    3. Cíl: zkrať tc ∧ bounded gradient

    Returns:
        regulated_values, diagnostics
    """
    ...
    if values.size > 1:
        grad_est = np.diff(regulated)
        max_grad = np.max(np.abs(grad_est)) if grad_est.size > 0 else 0.0
    else:
        max_grad = 0.0
    ...
    diagnostics = {
        ...
        "gradient_bounded": bool(max_grad <= 1.01),  # Allow small numerical slack
    }
...
def main() -> None:
    ...
    ap.add_argument("--ipc-weights", type=str, default="auto",
                    choices=["ones", "cvxopt", "corr", "cvxopt_corr", "auto"])
